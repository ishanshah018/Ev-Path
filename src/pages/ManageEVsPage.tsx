...
const ManageEVsPage: React.FC = () => {
  ...
  const batteryHealthColor = (health: number) => {
    if (health >= 90) return 'text-green-600';
    if (health >= 70) return 'text-yellow-600';
    return 'text-red-600';
  };

  const batteryHealthBg = (health: number) => {
                  ...
                  {formData.brand && (
                    <div>
                      ...
                      >
                        ...
                        {evModels[formData.brand as keyof typeof evModels]?.map((model) => (
